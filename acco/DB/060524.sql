...
-- Dumping data for table acco.booking: ~49 rows (approximately)
...
/*  SELECT bhk1count  into bhk1c  from temp_table WHERE NOT bookedstatusid=4 ORDER BY checkin DESC;
 SELECT bhk1count, bhk2count, bhk3count into bhk1c,  bhk2c, bhk3c from temp_table WHERE NOT bookedstatusid=4 ORDER BY checkin DESC;
 */ 
...
(
  ...
  `bhk3count` INT ,
 	`color` TEXT,
 	PRIMARY KEY (`indate`) USING BTREE
 	 );
/*
...
WHILE stage  DO
	...
	INSERT INTO temp_table(`indate`,`bhk2count`,`bhk3count`,`color`) VALUES(startdate,1,1,'red');
	...
	IF @dateDiffs>=1 then
		set stage=false;
	END If;
/*	CALL getguestdetailwithcolor(startdate);
...
-- Dumping data for table acco.logindetail: ~43 rows (approximately)

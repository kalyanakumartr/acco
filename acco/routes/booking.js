...
router.post('/checkinconfirm', function (req, res) {
  ...
  cmd='UPDATE booking b,room r SET b.bookedstatusid=3 ,b.roomid="'+req.body.roomid+'", b.roomnos="'+req.body.roomnos+'", b.acheckin="'+ain+'",r.status=9 WHERE b.bookingid=' + req.body.bookingid+' and r.roomid in ('+req.body.roomid+')' ;
    console.log(cmd);
  let data = [true, 1];
...
router.post('/addbookingwithchild', function (req, res) {
  try {
    // console.log("Body", req.body);
    img = "image.jpg";
    var command = sprintf('INSERT INTO booking (userid,modeoftypeid,roomtypeid,bhk1count,bhk2count,bhk3count,firstname,lastname,email,phonenumber,address1,address2,city,state,country,pincode,checkin,checkout,adults,child,childage,roomtype,bed,noofdays,price,totalprice,bookedstatusid,verificationstatus,imageUrl,status) VALUES   (%d,%d,%d,%d,%d,%d,"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s",%d,%d,"%s","%s","%s",%b)', req.body.userid, req.body.modeoftypeid,req.body.roomtypeid, req.body.bhk1count, req.body.bhk2count, req.body.bhk3count, req.body.firstname, req.body.lastname, req.body.email, req.body.phonenumber, req.body.address1, req.body.address2, req.body.city, req.body.state, req.body.country, req.body.pincode, req.body.checkin, req.body.checkout, req.body.adults, req.body.child, req.body.childage, req.body.roomtype, req.body.bed, req.body.noofdays, req.body.price, req.body.totalprice,1, req.body.verificationstatus, img, 1);
...
router.get('/getbookingwithuseridonly', authcheck, function (req, res) {
  try {
    // id=req.param.userid
    command = 'select * from booking WHERE userid=' + req.query.userid + ' AND NOT bookedstatusid=2 ORDER BY checkin desc';
    console.log(command);
    con.query(command, function (error, results) {
      if (error) {
        res.send("Unable to get Date ")

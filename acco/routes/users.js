...
router.post('/updatemarquee', authcheck, function (req, res) {
  try{
  console.log("Welcome to update user ");
  var id = req.body.marqueeId;
  var name = req.body.marqueeName;
  

  var command = 'UPDATE marquee SET marqueeName="' + name + '" WHERE marqueeId=' + id + '';
  console.log(command);
  let data = [true, 1];
  con.query(command, data, function (error, result) {
    ...
    if (result.affectedRows <= 0) {
      ...
      res.status(200).send({ message: "Successfully update Marquee" });
    };
  });
  }
  catch (e) {
